...
D) Mais de R$ 500 mil"""
    ...
    def _notify_consultant(self, lead_data: Dict, score: int) -> bool:
        ...
        try:
            ...
            # Enviar para N8N (que vai disparar email + CRM)
            n8n_result = n8n_service.send_qualified_lead(payload)

            if n8n_result['success']:
                logger.info("Consultor notificado via N8N", 
                           lead_id=lead_data['id'], 
                           score=score)
                return True
            else:
                logger.error("Falha ao notificar consultor", 
                           lead_id=lead_data['id'], 
