...
def start_backend():
    ...
    # Copiar configurações do backend
    env_backend = Path(".env.local.backend")
    env_target = Path("backend/.env")
    
    if env_backend.exists():
        import shutil
        shutil.copy2(env_backend, env_target)
        print("✅ Configurações do backend copiadas")
    else:
        print("⚠️  Arquivo .env.local.backend não encontrado!")
...
def start_frontend():
    ...
    # Copiar configurações do frontend
    env_frontend = Path(".env.local.frontend")
    env_target = Path("frontend/.env.local")
    
    if env_frontend.exists():
        import shutil
        shutil.copy2(env_frontend, env_target)
        print("✅ Configurações do frontend copiadas")
    else:
        print("⚠️  Arquivo .env.local.frontend não encontrado!")
